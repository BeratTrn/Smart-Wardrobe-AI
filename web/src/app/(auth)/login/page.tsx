...
export default function LoginPage() {
  ...
  // Google Identity Services — mobildeki google_sign_in akışının web karşılığı.
  // Script yüklenip initialize edildikten sonra gerçek Google butonunu
  // googleBtnRef'in içine render eder; tıklanınca dönen idToken backend'in
  // AYNI /api/auth/google ucuna gönderilir (mobille birebir aynı doğrulama).
  useEffect(() => {
    ...
    const renderGoogleButton = () => {
      ...
      window.google.accounts.id.initialize({
        client_id: clientId,
        callback: (response) => {
          googleAuth.mutate({ idToken: response.credential });
        },
      });
  ...
  return (
    ...
    >
      ...
      <div className="p-8 space-y-8">
        ...
        {/* Google ile giriş — gerçek Google Identity Services butonu */}
        {process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID ? (
          ...
          <button
            type="button"
            disabled
            title="NEXT_PUBLIC_GOOGLE_CLIENT_ID tanımlı değil"
