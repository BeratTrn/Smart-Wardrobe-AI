      ...
      {isPending ? (
          ...
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="glass rounded-2xl p-5 space-y-3">
              ...
              <div className="flex gap-2 pt-2">
                {[1, 2, 3].map((j) => (
                  <div key={j} className="skeleton h-12 w-12 rounded-lg" />
                ))}
              </div>
            </div>
          ))}
        ...
        <div className="glass rounded-2xl p-16 text-center">
          ...
          <p className="text-sm text-muted mt-1">
            Save outfits you love from the{" "}
            <a href="/outfits" className="text-gold hover:underline">Outfits page</a>
          </p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {outfits.map((saved) => (
            <OutfitResultCard
              key={saved._id}
              outfit={saved.kombId as unknown as Outfit}
              showRemove
              onRemove={handleRemove}
              isRemoveLoading={pendingRemoveId === saved._id}
            />
          ))}
        </div>
      )}
