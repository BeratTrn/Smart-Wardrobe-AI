    ...
    <div className="glass rounded-2xl p-6 space-y-5">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {SEASONS.map((s) => (
              <button key={s} type="button" onClick={() => setSeason(season === s ? "" : s)}
                className={`px-3 py-1.5 rounded-full text-[12px] border transition-colors ${
                  season === s ? "border-gold bg-gold/10 text-gold" : "border-border text-muted hover:border-gold/40"}`}>
                {s}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {EVENTS.map((ev) => (
              <button key={ev} type="button" onClick={() => setEvent(event === ev ? "" : ev)}
                className={`px-3 py-1.5 rounded-full text-[12px] border transition-colors ${
                  event === ev ? "border-gold bg-gold/10 text-gold" : "border-border text-muted hover:border-gold/40"}`}>
                {ev}
              </button>
            ))}
          </div>
        ...
        {weather && (
          <label className="flex items-center gap-3 cursor-pointer">
            <div role="checkbox" aria-checked={useWeather} onClick={() => setUseWeather((v) => !v)}
              className={`h-5 w-9 rounded-full transition-colors relative ${useWeather ? "bg-gold" : "bg-white/10"}`}>
              <span className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${useWeather ? "translate-x-4" : "translate-x-0.5"}`} />
            </div>
            <span className="text-[13px] text-muted">Use current weather ({weather.sehir}, {weather.sicaklik}°C)</span>
          </label>
        )}
