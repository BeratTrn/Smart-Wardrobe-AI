...
export function Topbar() {
  ...
  const initials = user
    ? `${(user.kullaniciAdi ?? user.ad ?? "S")[0]}`.toUpperCase()
    : "SW";

  return (
    <header className="flex h-16 items-center gap-4 px-4 md:px-6 border-b border-white/10 glass">
      <button className="lg:hidden text-muted hover:text-foreground" onClick={openMobileSidebar}>
        <Menu className="h-5 w-5" />
      </button>
      <div className="flex-1" />
      <button onClick={toggle} className="h-8 w-8 rounded-lg glass flex items-center justify-center text-muted hover:text-foreground text-xs" title="Toggle theme">
        {theme === "dark" ? "☀️" : "🌙"}
      </button>
      <button className="flex items-center gap-2.5 rounded-lg glass px-3 py-1.5 hover:bg-white/5 transition-colors">
        ...
        <div className="hidden sm:block text-left">
          <p className="text-[13px] font-medium leading-none">{user?.kullaniciAdi ?? user?.ad}</p>
